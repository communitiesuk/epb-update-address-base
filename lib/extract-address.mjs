export function extractAddress(addressBaseEntry) {
  const usesDeliveryPoint =
    addressBaseEntry.CLASS.startsWith('R') && addressBaseEntry.UDPRN;
  return usesDeliveryPoint
    ? createDeliveryPointAddress(addressBaseEntry)
    : createGeographicAddress(addressBaseEntry);
}
...
function createGeographicAddress(addressBaseEntry) {
  ...
  if (addressBaseEntry.PAO_TEXT !== '') {
    ...
    if (
      [
        ...
        'SAO_END_NUMBER',
        'SAO_END_SUFFIX',
      ].some((code) => addressBaseEntry[code] !== '')
    ) {
      line.push(
        [
          [
    ...
    street = [
      ...
      [addressBaseEntry.SAO_END_NUMBER, addressBaseEntry.SAO_END_SUFFIX].join(
        '',
      ),
    ]
      .filter((x) => x !== '')
  ...
  if (
    ...
    const pao = [
      ...
      [addressBaseEntry.PAO_END_NUMBER, addressBaseEntry.PAO_END_SUFFIX].join(
        '',
      ),
    ]
      .filter((x) => x !== '')
  ...
  lines.push(
    [street, addressBaseEntry.STREET_DESCRIPTION]
      .filter((x) => x !== '')
      .join(' '),
  );
  ...
  const postcode = addressBaseEntry.POSTCODE
    ? addressBaseEntry.POSTCODE
    : addressBaseEntry.POSTCODE_LOCATOR;
...
function combineStreetLine(lines) {
  return lines.reduce((carry, val) => {
    if (
      ...
      return carry
        .slice(0, -1)
        .concat([[carry.slice(-1)[0].toString(), val].join(' ')]);
    } else {
