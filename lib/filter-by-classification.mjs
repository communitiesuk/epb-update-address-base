export function isCertifiableAddress(address) {
  return (
    isInCertifiableCountry(address) && hasCertifiableClassification(address)
  );
}
...
function isInCertifiableCountry(address) {
  return !['L', 'M'].includes(address.COUNTRY); // filter out L=Channel Islands and M=Isle of Man
}
